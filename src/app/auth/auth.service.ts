...
export class AuthService {
      ...
      .subscribe(
        (response) => {
          ...
          if (token) {
            ...
            console.log(
              `customer documents created already exist: forwarding to homepage`
            );
  ...
  initCustomerDocs(token, expirationDate, firstLogin, custId) {
    ...
    console.log(custId);
    const userMenusDoc = this.http.post(
      BACKEND_URL + "accountServices/initMenuDoc",
      customer
    );
    const userIngredientsDoc = this.http.post(
      BACKEND_URL + "accountServices/initIngredientsDoc",
      customer
    );
    const userSuppliersDoc = this.http.post(
      BACKEND_URL + "accountServices/initSuppliersDoc",
      customer
    );
    const userDishesDoc = this.http.post(
      BACKEND_URL + "accountServices/initDishesDoc",
      customer
    );
    const userAccountValidated = this.http.post(
      BACKEND_URL + "accountServices/validateAccount",
      customer
    );
